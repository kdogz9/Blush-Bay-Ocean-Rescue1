# Blush-Bay-Ocean-Rescue 

## Game overview 

Blush Bay: Ocean Rescue is an aquarium management sim where players aim to restore the ocean by taking care of sick and injured marine life. 

Players will rescue the fish in a mini game where they click on a bubble. The fish will get placed in a tank where its stats can be seen. The tanks will have certain conditions to be met to keep the fish stabalised whilst undergoing treatment. Treating the fish will involve mini games. Players can grow their own ingredients in a lab which will then need to be placed in machines to create the fish food. They will acquire the ingredients through going diving and using sciccors to cut down seaweed and kelp etc. Visitors will look around the aquarium and donate based on the quality of the aquarium. When the marine life are all fully restored the player will be able to release them to be able to rescue more. Players will be able to buy new tanks and furniture from the shop with the donations given and decorate their aquarium however they like. 

This game will be 2D pixel art, top down approach like Stardew Valley and Fields of Misteria. 

## Research / applications to real life 

I wanted to create Blush Bay after taking inspiration from rehabilitation centres in America, specifically Atlana's one. Adding a real life application to the game makes it much more meaningful to cozy gamers as it brings light to the issues our oceans are currently facing. 

I've researched into aquarium floor plans and layouts to make the game as close to possible as a real aquarium. I've also looked into a lot of management style games to take inspiration for the game play and mechanics as well as the feel of the games too so I can apply this to my own game. These games include Two point museum and lets build a zoo/aqaurium DLC. 

## Game mechanics 

* Player movement (WASD)
* Visitor path finding (AI)
* Inventory system 
* Shop system - purchasing, availability, prices, upgrades 
* Donations - shell coins 
* Customisation - placing furniture, decorations, changing colours etc 
* Rescue system - mini game where players click on bubbles that pop up 
* Release system - cut scene ? button ? 
* Tank type - based on different fish 
* Tank conditions - cleaniness, filters, decorations (happiness level)
* Fish conditions - health, cleaniness, food requirements, happiness level 
* Lab grown ingredients for fish food - kelp, seaweed etc 
* Machines - fish food, filters , sanitising, reproducing 
* Treating the fish 
* Diving system to collect food 

## Future game mechanics 
* Breeding - nursery , coloured variations etc 
* Catalogue - sticker book UI
* More types of marine life 
* Upgrades of machinery, tank size etc 
* Rarity of fish rescued 
* more types of fish 
* more furniture 
* automatic treatment 